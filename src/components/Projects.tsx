...
const projects: Project[] = [
	{
		...
		title: "Flux",
		tagline:
			"Open-source desktop API client, a lightweight Postman alternative",
		kind: "Desktop App",
		...
		images: [
			{
				src: fluxApp,
				alt: "Flux request editor with collections and the AI test generator",
			},
		...
		links: [
			...
			{
				label: "Download",
				url: "https://github.com/angeldevmobile/Flux-Post/releases/latest",
				kind: "download",
			},
			{
				label: "Source",
				url: "https://github.com/angeldevmobile/Flux-Post",
				kind: "github",
			},
		],
	},
	{
		...
		images: [
			{
				src: fluxLearning,
				alt: "Flux Learning landing page with the deck generator",
			},
		],
		links: [
			{
	...
	},
	{
		...
		tagline:
			"A language for backend, automation and HPC, written in Rust end to end",
		...
		images: [
			{
				src: orionVscode,
				alt: "Orion code generating an Excel report, running in VS Code",
			},
		...
		links: [
			{
				label: "Documentation",
				url: "https://docs-orion.onrender.com/",
				kind: "docs",
			},
			{
				label: "Playground",
				url: "https://docs-orion.onrender.com/playground",
				kind: "play",
			},
			{
				label: "Source",
				url: "https://github.com/angeldevmobile/Orion",
				kind: "github",
			},
		],
	},
	{
		...
		links: [
			{
				label: "Source",
				url: "https://github.com/angeldevmobile/Seam",
				kind: "github",
			},
			{
				label: "crates.io",
				url: "https://crates.io/crates/seam-core",
				kind: "package",
			},
			{
				label: "PyPI",
				url: "https://pypi.org/project/seam-schema/",
				kind: "package",
			},
			{
				label: "npm",
				url: "https://www.npmjs.com/package/seam-schema",
				kind: "package",
			},
		],
	},
	{
		...
		images: [
			{
				src: orionMarketplace,
				alt: "Orion Language listing on the VS Code Marketplace",
			},
		...
		links: [
			...
			{
				label: "Source",
				url: "https://github.com/angeldevmobile/Extension-lenguaje-orion",
				kind: "github",
			},
		],
	},
	{
		...
		links: [
			...
			{
				label: "Source",
				url: "https://github.com/angeldevmobile/PlayGround---API",
				kind: "github",
			},
		],
	},
	{
		...
		images: [
			{
				src: smartRemote,
				alt: "SmartRemote remote screen with D-pad, volume and channel controls",
			},
			{ src: smartVoice, alt: "SmartRemote voice dictation screen" },
			{
				src: smartFeature,
				alt: "SmartRemote feature graphic: LG webOS, Android TV and Roku",
			},
		],
		links: [
			{
				label: "Source",
				url: "https://github.com/angeldevmobile/SmartController",
				kind: "github",
			},
		],
	},
	{
		...
		links: [
			{
				label: "Live site",
				url: "https://portfolio-angel-dev.onrender.com/",
				kind: "live",
			},
			{
				label: "Source",
				url: "https://github.com/angeldevmobile/Portfolio-Dev",
				kind: "github",
			},
		],
	},
...
const StatusBadge: React.FC<{ status: string; tone: StatusTone }> = ({
	status,
	tone,
}) => (
...
const TechChip: React.FC<{ name: string; large?: boolean }> = ({
	name,
	large,
}) => {
...
const CountUp: React.FC<{ to: number; suffix?: string }> = ({
	to,
	suffix = "",
}) => {
	...
	<div className={`pj-code ${large ? "pj-code--lg" : ""}`}>
		...
		<pre className="pj-code-body">
			<motion.span
				...
				<span className="tk-kw">schema</span>{" "}
				<span className="tk-ty">User</span> {"{"}
			</motion.span>
			{SEAM_FIELDS.map((f, i) => (
				<motion.span
					key={f.name}
					...
					{f.note && (
						<span className={f.comment ? "tk-cm" : "tk-at"}>{f.note}</span>
					)}
				</motion.span>
			))}
			<motion.span
...
const Preview: React.FC<{ project: Project; frame: number }> = ({
	project,
	frame,
}) => {
...
const ProjectCard = React.forwardRef<HTMLElement, CardProps>(
	({ project, size, order, onOpen }, ref) => {
		...
		const rotateX = useSpring(useTransform(ny, [0, 1], [5, -5]), {
			stiffness: 180,
			damping: 18,
		});
		const rotateY = useSpring(useTransform(nx, [0, 1], [-5, 5]), {
			stiffness: 180,
			damping: 18,
		});
		...
		const quickLinks = project.links
			.filter(
				(l) =>
					l.kind === "github" ||
					l.kind === "live" ||
					...
					<div className="pj-card-body">
						...
						<div className="pj-chip-row">
							...
							{project.stack.length > (size === "md" ? 4 : 6) && (
								<span className="pj-chip pj-chip--more">
									+{project.stack.length - (size === "md" ? 4 : 6)}
								</span>
							)}
						</div>
						...
						<div className="pj-card-foot">
							<div className="pj-card-foot-left">
								...
								<div className="pj-quick-links">
									...
									{project.privateRepo && (
										<span
											className="pj-quick-link pj-quick-link--muted"
...
const InstallBlock: React.FC<{ items: { label: string; cmd: string }[] }> = ({
	items,
}) => {
			...
			<motion.div
				...
				<div
					className={`pj-gallery ${
						project.preview === "phone" ? "pj-gallery--phone" : ""
					}`}>
					{count > 0 ? (
						<>
					...
					<div className="pj-modal-links">
						...
						{project.privateRepo && (
							<span
								...
								title="The source code is in a private repository">
			...
			<div className="pj-container">
				<motion.header
					...
					<p className="pj-subtitle">
						Developer tools, a programming language and products running in
						production. Every project here has a live link, published package or
